...
public class WmCheckPickHistorySqlProvider extends BaseSqlProvider {

	public String update(WmCheckPickHistory record) {
		BEGIN();
        UPDATE("wm_check_pick_history");
        if (record.getWmCheckPickHistoryId() != null) {
            SET("WM_CHECK_PICK_HISTORY_ID = #{wmCheckPickHistoryId}");
        }
        ...
        WHERE("wm_check_pick_history_id = #{wmCheckPickHistoryId}");

        return SQL();
	}

    public String selectAllByRecord(WmCheckPickHistory record) {
        BEGIN();
        SELECT("*");
        ...
        if (record.getWmCheckPickHistoryId() != null) {
            stringBuffer.append(" AND WM_CHECK_PICK_HISTORY_ID = #{wmCheckPickHistoryId}");
        }
        if (record.getType() != null) {
        ...
        if (record.getResultStatus() != null) {
            stringBuffer.append(" AND RESULT_STATUS = #{resultStatus}");
        }
        if (record.getActionDateTime() != null) {
