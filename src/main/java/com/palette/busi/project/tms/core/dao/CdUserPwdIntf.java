...
public interface CdUserPwdIntf extends BaseMybatisMapper {
	...
	@Select("SELECT  CD_USER_PWD_ID  AS cdUserPwdId, CD_USER_ID  AS cdUserId, USER_NAME  AS userName, SALT  AS salt, PWD  AS pwd, LAST_LOGIN_TIME  AS lastLoginTime, LAST_LOGIN_IP  AS lastLoginIp, COOKIE  AS cookie, COMPANY_CODE  AS companyCode, RECORD_VERSION  AS recordVersion, CREATE_USER_CODE  AS createUserCode, CREATE_DATE_TIME  AS createDateTime, CREATE_TIME_ZONE  AS createTimeZone, UPDATE_USER_CODE  AS updateUserCode, UPDATE_DATE_TIME  AS updateDateTime, UPDATE_TIME_ZONE  AS updateTimeZone FROM cd_user_pwd WHERE cd_user_pwd_id=#{cdUserPwdId}")
	public CdUserPwd selectCdUserPwdById(@Param("cdUserPwdId") int cdUserPwdId);
	
	@Select("SELECT  CD_USER_PWD_ID  AS cdUserPwdId, CD_USER_ID  AS cdUserId, USER_NAME  AS userName, SALT  AS salt, PWD  AS pwd, LAST_LOGIN_TIME  AS lastLoginTime, LAST_LOGIN_IP  AS lastLoginIp, COOKIE  AS cookie, COMPANY_CODE  AS companyCode, RECORD_VERSION  AS recordVersion, CREATE_USER_CODE  AS createUserCode, CREATE_DATE_TIME  AS createDateTime, CREATE_TIME_ZONE  AS createTimeZone, UPDATE_USER_CODE  AS updateUserCode, UPDATE_DATE_TIME  AS updateDateTime, UPDATE_TIME_ZONE  AS updateTimeZone FROM cd_user_pwd")
	public List<CdUserPwd> selectAllCdUserPwd();
	
	@Insert("insert into cd_user_pwd ( CD_USER_PWD_ID, CD_USER_ID, USER_NAME, SALT, PWD, LAST_LOGIN_TIME, LAST_LOGIN_IP, COOKIE, COMPANY_CODE, RECORD_VERSION, CREATE_USER_CODE, CREATE_DATE_TIME, CREATE_TIME_ZONE, UPDATE_USER_CODE, UPDATE_DATE_TIME, UPDATE_TIME_ZONE ) values (#{cdUserPwdId},#{cdUserId},#{userName},#{salt},#{pwd},#{lastLoginTime},#{lastLoginIp},#{cookie},#{companyCode},#{recordVersion},#{createUserCode},#{createDateTime},#{createTimeZone},#{updateUserCode},#{updateDateTime},#{updateTimeZone})")
