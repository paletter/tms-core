...
public interface CdUserRoleIntf extends BaseMybatisMapper {
	...
	@Select("SELECT  CD_USER_ROLE_ID  AS cdUserRoleId, CD_USER_ID  AS cdUserId, CD_ROLE_ID  AS cdRoleId, COMPANY_CODE  AS companyCode, RECORD_VERSION  AS recordVersion, CREATE_USER_CODE  AS createUserCode, CREATE_DATE_TIME  AS createDateTime, CREATE_TIME_ZONE  AS createTimeZone, UPDATE_USER_CODE  AS updateUserCode, UPDATE_DATE_TIME  AS updateDateTime, UPDATE_TIME_ZONE  AS updateTimeZone FROM cd_user_role WHERE cd_user_role_id=#{cdUserRoleId}")
	public CdUserRole selectCdUserRoleById(@Param("cdUserRoleId") int cdUserRoleId);
	
	@Select("SELECT  CD_USER_ROLE_ID  AS cdUserRoleId, CD_USER_ID  AS cdUserId, CD_ROLE_ID  AS cdRoleId, COMPANY_CODE  AS companyCode, RECORD_VERSION  AS recordVersion, CREATE_USER_CODE  AS createUserCode, CREATE_DATE_TIME  AS createDateTime, CREATE_TIME_ZONE  AS createTimeZone, UPDATE_USER_CODE  AS updateUserCode, UPDATE_DATE_TIME  AS updateDateTime, UPDATE_TIME_ZONE  AS updateTimeZone FROM cd_user_role")
	public List<CdUserRole> selectAllCdUserRole();
	
	@Insert("insert into cd_user_role ( CD_USER_ROLE_ID, CD_USER_ID, CD_ROLE_ID, COMPANY_CODE, RECORD_VERSION, CREATE_USER_CODE, CREATE_DATE_TIME, CREATE_TIME_ZONE, UPDATE_USER_CODE, UPDATE_DATE_TIME, UPDATE_TIME_ZONE ) values (#{cdUserRoleId},#{cdUserId},#{cdRoleId},#{companyCode},#{recordVersion},#{createUserCode},#{createDateTime},#{createTimeZone},#{updateUserCode},#{updateDateTime},#{updateTimeZone})")
