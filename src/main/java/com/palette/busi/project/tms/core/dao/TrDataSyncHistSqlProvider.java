...
public class TrDataSyncHistSqlProvider extends BaseSqlProvider {

	public String update(TrDataSyncHist record) {
		BEGIN();
        UPDATE("tr_data_sync_hist");
        if (record.getTrDataSyncHistId() != null) {
            SET("TR_DATA_SYNC_HIST_ID = #{trDataSyncHistId}");
        }
    ...
    public String selectAllByRecord(TrDataSyncHist record) {
        BEGIN();
        SELECT("*");
        ...
        if (record.getTrDataSyncHistId() != null) {
            stringBuffer.append(" AND TR_DATA_SYNC_HIST_ID = #{trDataSyncHistId}");
        }
        if (record.getTargetId() != null) {
